...
class HostedAgent(BaseAgent):
    ...
    async def start(self):
        ...
        try:
            ...
            print("Hosted Agent started successfully")
    ...
    async def _process_message(self, message: Message) -> Optional[str]:
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": """You are a Hosted Agent in the OpenPond P2P network.
...
- Reliable execution"""
    ...
    def _is_message_for_this_agent(self, message: Message) -> bool:
...
async def create_hosted_agent(
    ...
    """Create and start a hosted agent"""
    agent = HostedAgent(api_key, openai_api_key, api_url)
    await agent.start()
    return agent 